...
export function TeamLeaderOverview() {
  ...
  const [widgetOrder, setWidgetOrder] = useState(['metrics', 'trends', 'activity', 'leaderboard', 'coaching', 'insights']);
  ...
  const handleDrop = (e: React.DragEvent, targetWidgetId: string) => {
    ...
    if (draggedWidget && draggedWidget !== targetWidgetId) {
      console.log('Dropping', draggedWidget, 'on', targetWidgetId);
    ...
    <div className="flex-1 bg-gray-50 min-h-screen" onClick={handleClickOutside}>
      ...
      >
        ...
        <div className={`grid grid-cols-1 gap-6 ${expandedWidget === 'metrics' || expandedWidget === 'trends' ? 'lg:grid-cols-1' : 'lg:grid-cols-3'}`}>
          ...
          <SortableWidget id="metrics">
            <div 
              className={`${expandedWidget === 'metrics' ? 'lg:col-span-1' : 'lg:col-span-2'} ${expandedWidget === 'trends' ? 'hidden' : ''}`}
              onDragOver={handleDragOver}
              onDrop={(e) => handleDrop(e, 'metrics')}
            >
              <div className={`bg-white rounded-xl shadow-sm border border-gray-100 p-6 ${expandedWidget === 'metrics' ? 'min-h-[600px]' : ''} ${draggedWidget === 'metrics' ? 'opacity-50' : ''}`}>
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  >
                    <Move className="w-4 h-4 text-gray-400" title="Drag to reorder" />
                  </div>
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
          ...
          <SortableWidget id="trends">
            <div 
              className={`${expandedWidget === 'trends' ? 'lg:col-span-1' : 'lg:col-span-1'} ${expandedWidget === 'metrics' ? 'hidden' : ''}`}
              onDragOver={handleDragOver}
              onDrop={(e) => handleDrop(e, 'trends')}
            >
              <div className={`bg-white rounded-xl shadow-sm border border-gray-100 p-6 ${expandedWidget === 'trends' ? 'min-h-[600px]' : 'h-full'} ${draggedWidget === 'trends' ? 'opacity-50' : ''}`}>
              <div className="flex items-center justify-between mb-6">
